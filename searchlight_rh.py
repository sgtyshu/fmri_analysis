...
rh_mapping = CiftiReader(template_brain).get_stru_pos('CIFTI_STRUCTURE_CORTEX_RIGHT')[3]
# rh_mapping is the mapping list from 32494(including corpus callosum) to 29716 (excluding corpus callosum)
...
infl_mesh = '/nfs/h1/userhome/liyifan/workingdir/Raven-fmri/mesh/right.inflated_32k_fs_LR.surf.gii'
...
def mysearchlight(sub):
    ...
    X = X[29696:59412,:].T
    rh = np.zeros((72,32492))
    for index,data in enumerate(X):
        rh[index][rh_mapping] = X[index] 
    X = rh
    ...
    scores = scores[rh_mapping]
    raven_module.save_rh_as_dscalar(scores,'../result/searchlight_result',sub + '_searchlight_attributes_rh.dscalar.nii')
    print ('done with ' + sub)
